...
from totalsegmentator.map_to_binary import class_map, class_map_5_parts, class_map_parts_mr, \
...
def get_output_folder(task_id, trainer, plans, model):
    nnUNet_results = os.environ.get("nnUNet_results", None)
    folders = [f for f in os.listdir(nnUNet_results) if os.path.isdir(os.path.join(nnUNet_results, f))]
    # 判断id寻路径
    if task_id == 8:
        # 从folders中找到Dataset008开始的路径
        for folder in folders:
            if folder.startswith("Dataset008"):
                return os.path.join(nnUNet_results,folder, f"{trainer}__{plans}__{model}")
    else:
        for folder in folders:
            if folder.startswith(f"Dataset{task_id}"):
                return os.path.join(nnUNet_results, folder, f"{trainer}__{plans}__{model}")
    raise ValueError(f"Task {task_id} not found in nnUNet results folder")


def nnUNetv2_predict(task_id, model="3d_fullres", folds=None,
    ...
    import global_dir
    import shutil
    save_dir = os.path.join(global_dir.onnx_dir, f"{task_id}")
    os.mkdir(save_dir)
    torch.onnx.export(
        net,
        dummy_input,
        os.path.join(save_dir, f'task_{task_id}.onnx'),
        input_names=['input'],
        output_names=['output']
    )
    shutil.copy2(os.path.join(model_folder,"dataset.json"), save_dir)
    shutil.copy2(os.path.join(model_folder,"dataset_fingerprint.json"), save_dir)
    shutil.copy2(os.path.join(model_folder,"plans.json"), save_dir)
